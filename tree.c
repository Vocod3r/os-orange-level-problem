...
int tree_serialize(const Tree *tree, void **data_out, size_t *len_out) {
    ...
    for (int i = 0; i < sorted_tree.count; i++) {
        ...
        offset += (size_t)written + 1; // +1 for the '\0' sprintf writes
...
// Recursive helper: build and write a tree for a slice of sorted index entries.
//
// entries    — pointer into the index entries array
// count      — number of entries in this slice (all share the same parent dir)
// prefix_len — number of characters of each entry's path that are already
//              consumed by parent directories (i.e. skip the first prefix_len
//              chars to get the name relative to the current level)
...
static int write_tree_level(IndexEntry *entries, int count,
    ...
    while (i < count) {
        ...
        if (slash == NULL) {
            ...
            // ── Subdirectory: group all entries that share this dir name ──
            int dir_len = (int)(slash - rel);
            char dir_name[256];
            strncpy(dir_name, rel, (size_t)dir_len);
            dir_name[dir_len] = '\0';

            // Find the end of this subdirectory's entries
            int j = i;
            while (j < count) {
                const char *r = entries[j].path + prefix_len;
                if (strncmp(r, dir_name, (size_t)dir_len) == 0 &&
                    r[dir_len] == '/')
                    j++;
                else
                    break;
            ...
            // Recurse: new prefix_len includes "dir_name/"
            ObjectID subtree_id;
            int new_prefix = prefix_len + dir_len + 1; // +1 for '/'
            ...
            i = j; // skip past all entries we just recursed into
        }
    }
